...
import { showPotentialWinnerModal } from './help-modals';
...
function checkVictory(sorteoResult: SorteoResult, blockHeight: number): void {
    if (!authState.pubkey || !sorteoResult.winners) return;

    const isWinner = sorteoResult.winners.some((w: Bet) =>
    ...
    if (isConfirmed && blockHeight > effectiveLastCelebrated) {
        ...
        triggerVictoryCelebration(sorteoResult.winningNumber ?? 0, blockHeight);
...
export function renderResult(sorteoResult: SorteoResult, blockHeight: number): void {
    ...
    resultDisplayContainer.querySelector('#helpIcon')?.addEventListener('click', () => showTransparencyModal(sorteoResult.blockHash || '', blockHeight, sorteoResult.winningNumber ?? 0));
}
